...
Zo provides a set of React hooks that bridge the reactive graph with React's component model. These hooks let you subscribe to pulses, create computed values, run reactive side-effects, and use concurrent lanes — all within standard React components.
...
```tsx
import { PulseNode } from "zo";
import { usePulse } from "zo";
...
```tsx
import { PulseNode, useComputed } from "zo";
...
Runs a reactive side-effect inside a component that re-executes whenever its pulse dependencies change. This is the Zo equivelent of React's `useEffect`, but with automatic dependency tracking.

```tsx
import { PulseNode, useEffectPulse } from "zo";
...
```tsx
import { useScope, EffectNode, ERROR } from "zo";
...
Zo's equivalent of React's `useTransition`, implemented with concurrent lanes. Returns an `[isPending, startTransition]` tuple.
...
```tsx
import { PulseNode, useZoTransition, usePulse } from "zo";
...
```tsx
import { useLane, usePulse, PulseNode } from "zo";
